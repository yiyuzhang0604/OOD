...
public class MobileApp implements Comparable<MobileApp>{
  ...
  public MobileApp(String appName, MobileAppCategory appCategory, Float currentVersion, Float currentSize,
  ...
  /**
   * Helper method to calculate the total download across all of the OS
   * @return total downloads
   */
  private int getTotalDownload(){
    int total = 0;
    for(String os: this.getSupportedOS().keySet()){
      total += this.getSupportedOS().get(os);
    }
    return total;
  }
  ...
  public int compareTo(MobileApp o) {
    return this.getTotalDownload() - o.getTotalDownload();
  }
